...

{
  ...
  printf("triangle x%I64u with orientation %d:\n", (unsigned __int64) t->tri,
         t->orient);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%I64u  (%.12g, %.12g)\n",
...

{
  ...
  printf("subsegment x%I64u with orientation %d and mark %d:\n",
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%I64u  (%.12g, %.12g)\n",
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment dest  [%d] = x%I64u  (%.12g, %.12g)\n",
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < b->firstnumber) ||
        ...
        printf("Error:  Triangle %I64d has an invalid vertex index.\n",
               elementnumber);
        triexit(1);
      }
    }
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("Error:  Triangle %I64d is missing vertex %d in %s.\n",
  ...
  if (b->poly) {
    ...
    while (subsegloop.ss != (subseg *) NULL) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("Error:  Segment %I64d has no endpoints in %s.\n", segmentnumber,
               polyfilename);
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("Error:  Segment %I64d is missing its second endpoint in %s.\n",
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < b->firstnumber) ||
          ...
          printf("Error:  Segment %I64d has an invalid vertex index.\n", 
                 segmentnumber);
          triexit(1);
        }
      }
...

{
  ...
  if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
    ...
    if (m->badsubsegs.items == 1) {
      ...
      printf("  %I64d encroached subsegments, and therefore might not be truly\n"
             , m->badsubsegs.items);
    }
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    if (b->order == 1) {
      ...
      fprintf(outfile, "%4I64d    %4d  %4d  %4d", elementnumber,
      ...
      fprintf(outfile, "%4I64d    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber,
...

{
  ...
  while (subsegloop.ss != (subseg *) NULL) {
    ...
    if (b->nobound) {
      fprintf(outfile, "%4I64d    %4d  %4d\n", subsegnumber,
              vertexmark(endpoint1), vertexmark(endpoint2));
    } else {
      fprintf(outfile, "%4I64d    %4d  %4d    %4d\n", subsegnumber,
  ...
  if (regions > 0) {
    ...
    for (regionnumber = 0; regionnumber < regions; regionnumber++) {
      ...
      fprintf(outfile, "%4I64d   %.17g  %.17g  %.17g  %.17g\n",
...

{
  ...
  *vpointmarkerlist = (int *) NULL;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    fprintf(outfile, "%4I64d    %.17g  %.17g", vnodenumber, circumcenter[0],
            circumcenter[1]);
    for (i = 2; i < 2 + m->nextras; i++) {
      /* Interpolate the vertex attributes at the circumcenter. */
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (trisym.tri == m->dummytri) {
          ...
          fprintf(outfile, "%4I64d   %d  %d   %.17g  %.17g\n", vedgenumber,
...

{
  ...
  printf("  Mesh triangles: %I64d\n", m->triangles.items);
  printf("  Mesh edges: %I64d\n", m->edges);
  printf("  Mesh exterior boundary edges: %I64d\n", m->hullsize);
  if (b->poly || b->refine) {
    printf("  Mesh interior boundary edges: %I64d\n",
           m->subsegs.items - m->hullsize);
    printf("  Mesh subsegments (constrained edges): %I64d\n",
           m->subsegs.items);
  }
  printf("\n");
  ...
  if (b->verbose) {
    ...
    printf("  Maximum number of vertices: %I64d\n", m->vertices.maxitems);
    printf("  Maximum number of triangles: %I64d\n", m->triangles.maxitems);
    if (m->subsegs.maxitems > 0) {
      printf("  Maximum number of subsegments: %I64d\n", m->subsegs.maxitems);
    }
    if (m->viri.maxitems > 0) {
      printf("  Maximum number of viri: %I64d\n", m->viri.maxitems);
    }
    if (m->badsubsegs.maxitems > 0) {
      printf("  Maximum number of encroached subsegments: %I64d\n",
             m->badsubsegs.maxitems);
    }
    if (m->badtriangles.maxitems > 0) {
      printf("  Maximum number of bad triangles: %I64d\n",
             m->badtriangles.maxitems);
    }
    if (m->flipstackers.maxitems > 0) {
      printf("  Maximum number of stacked triangle flips: %I64d\n",
             m->flipstackers.maxitems);
    }
    if (m->splaynodes.maxitems > 0) {
      printf("  Maximum number of splay tree nodes: %I64d\n",
             m->splaynodes.maxitems);
    }
    printf("  Approximate heap memory use (bytes): %I64d\n\n",
    ...
    if (!b->weighted) {
      printf("  Number of incircle tests: %I64d\n", m->incirclecount);
    } else {
      printf("  Number of 3D orientation tests: %I64d\n", m->orient3dcount);
    }
    printf("  Number of 2D orientation tests: %I64d\n", m->counterclockcount);
    if (m->hyperbolacount > 0) {
      printf("  Number of right-of-hyperbola tests: %I64d\n",
             m->hyperbolacount);
    }
    if (m->circletopcount > 0) {
      printf("  Number of circle top computations: %I64d\n",
             m->circletopcount);
    }
    if (m->circumcentercount > 0) {
      printf("  Number of triangle circumcenter computations: %I64d\n",
             m->circumcentercount);
    }
    printf("\n");
  }
...

{
  ...
  if (!b.quiet) {
    ...
    if (b.usesegments && !b.refine) {
      printf("Segment milliseconds:  %I64d\n",
...
#ifndef CDT_ONLY
    if (b.quality) {
      printf("Quality milliseconds:  %I64d\n",
...
#ifndef NO_TIMER
    gettimeofday(&tv6, &tz);
    printf("\nOutput milliseconds:  %I64d\n",
           1000l * (tv6.tv_sec - tv5.tv_sec) +
           (tv6.tv_usec - tv5.tv_usec) / 1000l);
    printf("Total running milliseconds:  %I64d\n",
